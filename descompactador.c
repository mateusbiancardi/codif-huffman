...
Descompactador* criaDescompactador(const char* caminho_entrada) {
    ...
    //duplica a string do caminho de entrada pro descompactador ter sua própria cópia
    d->arqEntrada = strdup(caminho_entrada);

  //verfica se tem a extensão .comp, se tiver, aloca memoria pro novo nome (sem o .comp)
    size_t len = strlen(caminho_entrada);
    if (len > 5 && strcmp(caminho_entrada + len - 5, ".comp") == 0) {
        d->arqSaida = (char*)malloc(len - 4); // len - 5 para remover ".comp", +1 para '\0'
        if (d->arqSaida == NULL) {
...
static Arvore* leCabecalho(FILE* arq) {
    ...
    if (tipo == '1') {    //no folha
        ...
        return criaNoInterno(esquerda, direita);   //conecta os nós filhos e retorna o nó pai
    }
...
static void descompactaDados(Descompactador* d, FILE* arq_orig, FILE* arq_saida) {
    if (!d || !d->arvore) return;

    Arvore* noAtual = d->arvore;
    int byte_lido;

    while ((byte_lido = fgetc(arq_orig)) != EOF) {  //le o arquivo byte por byte
        for (int i = 7; i >= 0; i--) {   //itera sobre cada um dos 8 bits do byte lido
            int bit = (byte_lido >> i) & 1;  //extrai o bit mais significativo primeiro
            ...
            if (ehNoFolha(noAtual)) {
                ...
                fputc(getCaractere(noAtual), arq_saida);   //escreve o caracter no arquivo de saída
...
void executaDescompactacao(Descompactador* d) {
    ...
    FILE* arq_entrada = fopen(d->arqEntrada, "rb");   //abre o arquivo pra leitra binaria
    if (arq_entrada == NULL) {
    ...
    FILE* arq_saida = fopen(d->arqSaida, "wb");  //abre um nobo arquivo pra escrever binario
    if (arq_saida == NULL) {
